                            ...
                            <div className="flex flex-col md:flex-row print:flex-row justify-between items-start md:items-center gap-2 md:gap-6 border-b border-gray-50 pb-2 md:pb-4">
                                ...
                                <div>
                                    <Image
                                        ...
                                        loading="eager"
                                    />
                                    <p className="leading-9 text-sm text-black">{order?.sbAddress.city}, {order?.sbAddress.region}, Bangladesh.</p>
                                </div>
                            </div>
                            <div className="flex flex-col md:flex-row justify-between items-start gap-2 md:gap-6 pt-2 md:pt-4">
                                <div className="text-sm text-black">
                                    <h4 className="uppercase text-gray-600 text-base font-bold leading-7">Date</h4>
                                ...
                                <div className="text-sm text-black">
                                    <h4 className="uppercase text-gray-600 text-base font-bold leading-7">Invoice No</h4>
                                    <p>{order?.invoice}</p>
                                </div>
                                <div className="text-start md:text-end text-sm text-black">
                                    <h4 className="uppercase text-gray-600 text-base font-bold leading-7">Invoice To</h4>
                            ...
                            <div className="-my-2 overflow-x-auto">
                                <table className="table-auto min-w-full border border-gray-100 divide-y divide-gray-200">
                                    <thead className="bg-base-300">
                                        <tr className="text-sm font-semibold text-gray-900 uppercase tracking-wider">
                                            <th scope="col" className="px-6 py-2 text-left">Sr.</th>
                                            <th scope="col" className="px-6 py-2 text-left">Product Name</th>
                                            <th scope="col" className="px-6 py-2 text-center">Quantity</th>
                                            <th scope="col" className="px-6 py-2 text-center">Item Price</th>
                                            <th scope="col" className="px-6 py-2 text-right">Amount</th>
                                        </tr>
                                    </thead>
                                    <tbody className="bg-white divide-y divide-gray-100 text-sm">
                                        {order?.cart.map((item, id) => <tr key={id}>
                                            <th className="px-6 py-1 whitespace-nowrap font-semibold text-black text-left">{id + 1}</th>
                                            <td className="px-6 py-1 whitespace-nowrap font-semibold text-black">{item.title}</td>
                                            <td className="px-6 py-1 whitespace-nowrap font-bold text-center">{item.quantity}</td>
                                            <td className="px-6 py-1 whitespace-nowrap font-bold text-center">${item.price.toFixed(2)}</td>
                                            <td className="px-6 py-1 whitespace-nowrap text-right font-bold text-red-500">${(item.price * item.quantity).toFixed(2)}</td>
                                        </tr>)
                                        }
                                    </tbody>
                                </table>
                        ...
                        <div className="border-t border-b border-gray-100 p-5 lg:p-10 bg-emerald-50 print:bg-transparent print:border-0">
                            <div className="flex lg:flex-row md:flex-row flex-col justify-between print:grid print:grid-cols-2 pt-4">
                                <div className="mb-3 md:mb-0 lg:mb-0 flex flex-col sm:flex-wrap">
                                    <span className="mb-1 font-bold text-sm uppercase text-gray-700 block">Payment Method</span>
                                    <span className="text-lg text-black font-semibold block">{order?.paymentMethod}</span>
                                </div>
                                <div className="mb-3 md:mb-0 lg:mb-0 flex flex-col sm:flex-wrap">
                                    <span className="mb-1 font-bold text-sm uppercase text-gray-700 block">Shipping Cost</span>
                                    <span className="text-lg text-black font-semibold block">${order?.shippingCost.toFixed(2)}</span>
                                </div>
                                <div className="mb-3 md:mb-0 lg:mb-0 flex flex-col sm:flex-wrap">
                                    <span className="mb-1 font-bold text-sm uppercase text-gray-700 block">Discount</span>
                                    <span className="text-lg text-black font-semibold block">${order?.discount.toFixed(2)}</span>
                                </div>
